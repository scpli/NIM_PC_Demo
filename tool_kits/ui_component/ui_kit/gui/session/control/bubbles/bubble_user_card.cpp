#include "stdafx.h"
#include "bubble_user_card.h"

using namespace ui;

namespace nim_comp
{
	void MsgBubbleUserCard::InitControl(bool bubble_right)
	{
		...
		msg_sticker_->AttachMenu(nbase::Bind(&MsgBubbleUserCard::OnMenu, this, std::placeholders::_1));
	}

	void MsgBubbleUserCard::InitInfo(const nim::IMMessage &msg)
	{
		...
		if (StringToJson(msg.attach_, json) && json.isObject())
		{
			...
			if (sub_type == CustomMsgType_UserCard && json["data"].isObject()) //finger
			{
	...
	bool MsgBubbleUserCard::OnMenu(ui::EventArgs* arg)
	{
